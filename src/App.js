...
class App extends Component {
  ...
  changeCategory = tabId => {
    const result = tabsList.filter(each => each.tabId === tabId)

    this.setState({
      tabIdValue: result[0].tabId,
    })
  }
  ...
  getListOfSections = () => (
    <ul className="buttons-container">
      {tabsList.map(each => (
        <li className="category-element" key={each.tabId}>
          <button type="button" onClick={() => this.changeCategory(each.tabId)}>
            {each.displayText}
          </button>
        </li>
      ))}
  ...
  verify = id => {
    ...
    if (imgUrl === result[0].imageUrl) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        imgUrl: imagesList[randomIndex].imageUrl,
      }))
    } else {
      this.secondCard()
    }
  ...
  render() {
    const {imgUrl, score, time, tabIdValue} = this.state
    const isTimePositive = time > 0
    const finalList = imagesList.filter(each => each.category === tabIdValue)
    return (
          ...
          {isTimePositive ? (
            <div>
              ...
              <ul className="images-container">
                {finalList.map(each => (
                  <li className="list-element" key={each.id}>
                    <button type="button" onClick={() => this.verify(each.id)}>
                      <img
